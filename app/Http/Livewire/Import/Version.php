...
class Version extends Component
{
    ...
    {
        return view('livewire.import.version');
    }
    ...
    public function simpanData()
    {
        ...
        try {
            activity()->causedBy(HelperController::user())->log("Melakukan import data version");
            // (new ImportSales)->import($this->file, 'local', ExcelSecond::XLSX);
            $import = new ImportVersion;
            $import->import($this->file);
            $message = "Berhasil melakukan import data version";
            $this->file = null;
            // return session()->flash('success', $message);
            return redirect()->route('version')->with('success', $message);
